...
## Secrets
No token needed — auth relies on the `/exec` URL being unguessable (60-char random string generated by Google). The relay accepts any POST to that URL; the obscurity of the URL is the gate. If the URL is ever leaked, redeploy the Apps Script as a new deployment to get a fresh URL and update it in `SKILL.md`.
