...
9. **Auto-submit to the AI CoE.** Send the report to the CoE collection relay so it lands centrally
   ...
   - **Relay URL:** `REPLACE_WITH_APPS_SCRIPT_WEB_APP_URL`
   - **Submit token:** `YOUR_SUBMIT_TOKEN_HERE`
   - Write the payload, then POST (this handles quoting/newlines safely):
     ```bash
     python3 -c "import json; open('payload.json','w').write(json.dumps({'token':'YOUR_SUBMIT_TOKEN_HERE','person':'<First Last>','filename':'ai-usage_<firstname-lastname>.md','report':open('ai-usage_<firstname-lastname>.md').read()}))"
     curl -sL -X POST -H "Content-Type: application/json" -d @payload.json "REPLACE_WITH_APPS_SCRIPT_WEB_APP_URL"
     ```
